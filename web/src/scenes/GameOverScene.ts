...
export class GameOverScene extends Phaser.Scene {
  ...
  create(data: { score: number; stage: number; level: number; isRecord?: boolean }): void {
    // 重建时沿用第一次算出来的结果，否则新纪录会被自己盖掉
    const isRecord = data.isRecord ?? saveHighScore(data.score);
    restartOnResize(this, () => ({ score: data.score, stage: data.stage, level: data.level, isRecord }));
    ...
    neonText(this, GAME_W / 2, y(0.46), isRecord ? '★ 新纪录 ★' : `HI-SCORE ${loadHighScore()}`, 22, COLORS.magenta);
    const prompt = neonText(this, GAME_W / 2, y(0.56), '空格 / 点击 重新开始', 24, COLORS.white);
    neonText(this, GAME_W / 2, y(0.60), 'M 返回菜单', 18, COLORS.purple);
